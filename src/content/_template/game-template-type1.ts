...
// ============================================================
// CẤU HÌNH GAME TYPE 1 - MULTI-SCENE FUNCTION TESTING
// ============================================================
...
export default function initGame(
  ...
  const updateTestCaseTable = () => {
    ...
    testResults.forEach((result, index) => {
      ...
      const outputDiff = renderGameOutputDiff(result.expected, result.actual);
      row.dataset.outputDiffApplied = "true";
      row.innerHTML = `
        <td>Scene ${index + 1}</td>
        <td class="input">${result.input}</td>
        <td class="input">${outputDiff.expectedHtml}</td>
        <td class="output">${outputDiff.actualHtml}</td>
  ...
  nextSceneBtn.addEventListener("click", () => {
    ...
    if (currentScene < GAME_CONFIG.testCases.length - 1) {
      currentScene++;
      updateSceneProgress();
